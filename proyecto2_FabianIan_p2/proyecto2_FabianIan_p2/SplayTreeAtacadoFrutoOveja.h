#ifndef SPLAYTREEATACADOFRUTOOVEJA_H
#define SPLAYTREEATACADOFRUTOOVEJA_H
...
class SplayTreeAtacadoFrutoOveja {

public:

    // Constructores
    SplayTreeAtacadoFrutoOveja(int pTiempoDestruccion, int tasaComer, vector<SplayTree>* pArbol)
            ...
            for (int pos = 0; pos < arbol->size(); pos++) {
                if ((*arbol)[pos].getVivo()) {
                    if ((*arbol)[pos].destruccionFrutoCompleta() && !(*arbol)[pos].getAtacadoFrutoCuervo()) {
                        ...
                        cout << "Splay Frutos restantes (Oveja): " << (*arbol)[pos].CountNodes((*arbol)[pos].raiz) << endl;
                    }
                }
            }
        }
