#ifndef HEAPTREEATACADOFRUTOCUERVO_H
#define HEAPTREEATACADOFRUTOCUERVO_H
...
class HeapTreeAtacadoFrutoCuervo {

public:

    // Constructores
    HeapTreeAtacadoFrutoCuervo(int pTiempoDestruccion, int tasaComer, vector<Heap>* pArbol)
            ...
            for (int pos = 0; pos < arbol->size(); pos++) {
                if ((*arbol)[pos].getVivo()) {
                    if ((*arbol)[pos].destruccionFrutoCompleta() && (*arbol)[pos].getAtacadoFrutoCuervo()) {
                        ...
                        cout << "Heap Frutos restantes (Cuervo): " << (*arbol)[pos].cantidad << endl;
                    }
                }
            }
        }
